...
//     `https://graph.facebook.com/v20.0/${id}?access_token=EAAYbZBkW0wTYBO6wjvw5yoE1ZBSXLL3o92TfIu3bJiiZBHnx2j0tYpZA69BNyTm5Hx2p6FhGWr7LGydalEBV5JGS3cmVKmVxsmSvFImNpCxeXxS4n2lDuP5BwxW6vGuTUqLB7ik9UI7VpEJb3163v4uzcPnFbhuPyd14jdttpZCB53uFSGqGen5sJuoA0Q04nWTxGy40hXrqKGw0ZBDKnu3TqwzRkPl2zrZCEQPFDZANHRsZD`
//   );
//   if (!response.ok) {
...
//     try {
//       const documentUrl = document ? await handleUpload(document, "documents") : "";
...
//         if (!response.ok) {
//           throw new Error(`Failed to send message: ${data.error?.message || response.statusText}`);
//         }
//       }
...
//   return (
//     <div className="flex flex-col lg:flex-row-reverse lg:space-x-reverse lg:space-x-6 p-6 space-y-6 lg:space-y-0">
...
const fetchTemplateDetails = async (id) => {
  const response = await fetch(
    `https://graph.facebook.com/v20.0/${id}?access_token=EAAYbZBkW0wTYBO8MufpJln3szUjyPx8aesb2USJgmYgd9jnqoOwTA7lGASvmv9sVtEDUyQNTZC3KAtZCj6im6eZAtdFYYxeRe0Hag86tUP8ODmNUR7s5uI1VavN712iuUpBAyQPZCCQOsMXu5oX0UY72B8kAvy1L65Er2XoATfT0CFAzOELTzVnL3YuYsfMSXogZDZD`
  );
  if (!response.ok) {
    throw new Error("Failed to fetch template details");
  }
