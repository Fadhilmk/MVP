...
//         <h1 className="text-2xl font-bold mb-2 text-white">{templateDetails.name}</h1>
//         <p><strong>Category:</strong> {templateDetails.category}</p>
//         <p><strong>Status:</strong> {templateDetails.status}</p>
...
const fetchTemplateAnalytics = async (templateId, startDate, endDate) => {
  try {
    const response = await fetch(
      `https://graph.facebook.com/v20.0/321322121074843/template_analytics?start=1721661380&end=1724359580&granularity=daily&metric_types=cost,clicked,delivered,read,sent&template_ids=[${templateId}]`,
      {
        method: "GET",
        headers: {
